        ...
        {hook name="checkout:payment_method"}
            {foreach from=$payments item="payment"}
                ...
                <li class="ty-payments-list__item">
                    <input id="payment_{$payment.payment_id}" class="ty-payments-list__checkbox cm-select-payment" type="radio" name="payment_id" value="{$payment.payment_id}" {if $payment_id == $payment.payment_id}checked="checked"{/if} {if $payment.disabled}disabled{/if} />
